...
go :- set_prolog_flag(occurs_check, true), time(call_with_time_limit(20, limit(1, (not_subtype(ia33554436(A),B), not_subtype(C,B), not_subtype(B,ia33554436(B)), not_subtype(D,ia33554436(B)), format('~w ~w~n', ['A =', A]),format('~w ~w~n', ['B =', B]),format('~w ~w~n', ['C =', C]),format('~w ~w~n', ['D =', D]))))).
